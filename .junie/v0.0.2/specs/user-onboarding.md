...
- `onboardingRequired` endpoint should return a response with
    ...
    table and onboarding is required. The flag should be set to `false`
    otherwise.
...
- `onboardUser` end-point should be publicly accessible, but ONLY when there are
    no users in the `users` table. If there are any users in the database
    already, the end-point should return a 401 Unauthorized response.
...
- Show a snackbar with an error message when the onboarding fails. This Snackbar
    should stay on screen until the clicks `OK` button or tries to submit the
    form again.
