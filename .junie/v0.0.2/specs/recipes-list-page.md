...
- The current list of recipes should be an Observable, which takes current
    filters as an input and then makes a call to the backend API -
    `/api/recipes` defined in `api` project.
...
- Each filter field should have a corresponding setter method to update filters
    Subject. When cuisine, category, or search are changed, the page number
    should reset to 1. If the page changes, no other filters should change.
...
- To populate cuisine and category filter options, the service should make a
    call to the backend API - `/api/recipes/cuisines-categories` defined in
    `api` project. The result should be stored in a private readonly
    BehaviourSubject. A getter method should return the BehaviourSubject
    containing cuisine and category options as an Observable, and a reload
    method should be added to refresh the list of options when needed.
...
- A list of recipes should be displayed in a table showing name, description,
    cuisine, and category for desktop clients. For mobile clients only recipe
