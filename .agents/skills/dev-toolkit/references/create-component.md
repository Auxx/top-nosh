...
Create a new Angular component and scaffold component code.

## Key Principles

1. **Always use `--no-interactive`** - Prevents prompts that would hang execution
2. Angular component generator is invoked using `nx g @top-nosh/dev-toolkit:component` followed by required arguments
...
- New component name: must be a non-empty string (convert to camel case and pass to generator as `--name` argument)
...
```bash
nx g @top-nosh/dev-toolkit:component --project=<project-name> --feature=<feature-name> --name=<component-name> --no-interactive --dry-run
```
...
```bash
nx g @top-nosh/dev-toolkit:component --project=<project-name> --feature=<feature-name> --name=<component-name> --no-interactive
```
...
Then verify that the generated code works. Keep in mind that the changes you make with a generator or subsequent modifications might impact various projects, so it's usually not enough to only run targets for the artefact you just created.
