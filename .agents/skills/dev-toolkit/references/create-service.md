...
Create a new Angular services and scaffold service code.

## Key Principles

1. **Always use `--no-interactive`** - Prevents prompts that would hang execution
2. Angular service generator is invoked using `nx g @top-nosh/dev-toolkit:service` followed by required arguments
...
- New service name: must be a non-empty string (convert to camel case and pass to generator as `--name` argument)
...
```bash
nx g @top-nosh/dev-toolkit:service --project=<project-name> --feature=<feature-name> --name=<service-name> --no-interactive --dry-run
```
...
```bash
nx g @top-nosh/dev-toolkit:service --project=<project-name> --feature=<feature-name> --name=<service-name> --no-interactive
```

### 4. Modify Generated Code (If Needed)

Service generator provides a starting point. Modify the output as needed to:
...
Then verify the generated code works. Keep in mind that the changes you make with a generator or subsequent modifications might impact various projects, so it's usually not enough to only run targets for the artefact you just created.
