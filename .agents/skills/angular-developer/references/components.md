...
To avoid calling a signal multiple times inside a template, cache it at the start of the template.

```html
<!-- ❌ INCORRECT: signal is called multiple times inside one template -->
@if (icon() !== null) {
<p>Icon is {{ icon() }}.</p>
}
```
...
```html
<!-- ✅ CORRECT: cache the value and re-use it -->
@let iconValue = icon;

@if (iconValue !== null) {
  <p>Icon is {{ iconValue }}.</p>
}
```
